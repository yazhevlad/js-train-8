...
function getUsersFromDatabase(array) {
  ...
  // return newArr;
  const newArr = Array.from(array, (user) => ({
    id: user.id,
...
function findElementIndexes(arr, element) {
  ...
  const first = arr.indexOf(element);
  const last = arr.lastIndexOf(element);
  const newArr = [];
...
function customIncludes(arr, element) {
  ...
  const includesElem = arr.includes(element);

  const filteredElem = arr.filter((item) => item === element).length;
